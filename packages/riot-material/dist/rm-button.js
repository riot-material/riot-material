(function (factory) {
    if (typeof module === "object" && typeof module.exports === "object") {
        ...
        define(["require", "exports", "riot", "./ripple"], factory);
    }
        ...
        'exports': {
            onMounted: function () {
                ...
                button.addEventListener("pointerdown", function (event) {
                    ...
                    _this.update({ pressed: true });
                    var cancel = function (ev) {
                        if (ev.pointerId !== event.pointerId) {
                            return;
                        }
                        _this.update({ pressed: false });
                ...
                var rippleOptions = this.isIcon() ? { centered: true, unbounded: true, radius: 20 } : {};
                ...
                Object.entries(this.props).forEach(function (entry) {
                    var key = entry[0], value = entry[1];
                    var rippleKeyMatch = key.match(/ripple([A-Z]\w+)/);
                    if (!rippleKeyMatch) {
                        return;
                    }
                    var rippleKey = rippleKeyMatch[1];
                ...
                var rippleObj = ripple_1.ripple(button, rippleOptions);
                var shouldBeClick = false;
                ...
                var openOverlay = this.props.openOverlay;
                if (openOverlay) {
                    button.addEventListener("click", function () {
                        var overlay = document.querySelector("#" + openOverlay);
                        if (!overlay) {
                            return;
                        }
                        var tag = overlay[riot.__.globals.DOM_COMPONENT_INSTANCE_PROPERTY];
                        if (tag && tag.open) {
                            tag.open();
                        }
                    });
